...
配置复用；Codex 可以导入 Codex CLI 的 `auth.json`，也可以直接使用设备登录；
Gemini 可以使用 API Key，或导入 Gemini CLI 的 `oauth_creds.json`。OAuth Token
只在控制平面刷新，绝不会进入候选容器、事件、工具结果和运行归档。无论 Profile
填写什么 BaseURL，Codex Token 都只能发往 OpenAI 官方认证/Codex 域名，Gemini
Token 都只能发往 Google OAuth/Code Assist 域名。
...
阶段时间线、周期资源快照、错误流、完整调查图谱与产物 SHA-256 清单。OAuth、
API Key、Authorization、密码和 Gemini Thought Signature 会被排除或脱敏。
...
- **Codex JSON 导入：** 上传 Codex CLI 生成的 `~/.codex/auth.json`。平台兼容
  标准的 `tokens` 嵌套结构与归一化后的扁平 OAuth 结构。
...
保存凭据后，新建或编辑模型配置，选择 **Codex OAuth（ChatGPT 订阅）**或
**Google Gemini 原生 API**，再选择匹配的凭据。UI 与 API 都会拒绝不兼容组合。
Gemini API Key 使用标准 Generative Language 端点；Gemini OAuth 使用 Code
Assist 端点，并可能要求 Gemini CLI 已经完成账户初始化。

`auth.json` 与 `oauth_creds.json` 包含 Refresh Token，安全等级等同密码。只能导入
到你自己控制的 EvilBench 实例；必须长期保管同一份 `APP_SECRET`，远程部署必须
使用 HTTPS。Codex 订阅认证与 Platform API Key 是两种不同能力，在本项目中标为
实验性；部署者应自行确认账户、组织策略与适用服务条款允许这种用法。
