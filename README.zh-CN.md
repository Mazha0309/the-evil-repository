...
The Evil Repository 会把模型扔进一场确定但充满不确定性的生产事故：两个相互
引用的 Git 仓库、彼此矛盾的文档、损坏的 CI 预言机、间歇服务症状、不存在的
Bug、Prompt Injection、脚本化工具故障、过时的 SQLite 缓存，以及塞满脏数据的
生产风格 PostgreSQL 快照。模型必须先判断什么是真的、什么绝不能改、如何控制
风险，然后才提交最小且经过证明的补丁与可审计证据报告。
...
场景 3.0.0 让规模本身进入有效调查面：五条真实中继链包含 704 个不透明执行
