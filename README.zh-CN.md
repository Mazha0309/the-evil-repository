...
一次运行除了 1,200 分 Scorecard，还会产出 Hypothesis / Evidence / Truth Graph、
行为画像、离散错误统计和调查过程回放。
...
1. 版本化的 Scenario SDK 目录包，包含仓库、数据库、注入、确定性故障、离线
   Browser 语料、隐藏真相、评分规则与元数据；
2. 与具体题目无关的 Runner，统一执行
...
场景 2.0.0 让规模本身进入有效调查面：192 个 compatibility ledger shard 与
48 个 query fragment 会在真实运行路径中执行；约 192 次相关历史变更互相冲突；
双仓库还有 224 组交叉引用的 incident bundle，共形成 1,344 份
CI / 日志 / Issue / migration / runtime 记录。
...
标准 completion 下限为 240 次实质工具调用、6 个假设（其中 3 个必须被否决）、
覆盖 Git / 数据库 / Browser / 运行时的 24 条证据、全部必需调查动作，以及
2,500 字符的报告。填充调用和重复读取会被审计并扣分。

标准题的难度校准目标是：强软件工程 Agent 的参考解题过程至少约 80 分钟。这是
校准目标，不是墙上时间承诺；EvilBench 不会用 `sleep`、随机延迟或强制等待
80 分钟来充数。难度必须来自不可省略的证据工作、冲突来源、对脚本化故障的有界
恢复和隐藏验收。一旦强 Agent 找到实质捷径，就应升级场景版本并重新校准。
