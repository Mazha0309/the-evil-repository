...
The Evil Repository 是一个面向 AI Agent 仓库级软件事故响应的开源 Benchmark
与行为分析平台。它刻意比单纯的「生成补丁」Benchmark 更宽，重点包括：
...
第一个标准场景是一场生产风格 CI 回归事故，包含两个 Git 仓库、一个过时的 SQLite
...
「Evil」是项目名称，不是科学结论。发布说明应强调真实性、生产约束、事故响应、
证据质量与仓库规模，而不是宣传题目有多折磨。

当前平台只有一个公开开发场景和一个活跃题族，属于工程试验台，还不是统计有效的
通用排行榜。Suite 是否就绪必须由机器可读策略明确表达，不能靠宣传文案决定。
...
- 复制或再分发受版权保护的网站内容。
- 宣称共享内核容器是绝对安全边界。
- 把一个公开场景或同一因果结构的大量种子副本，当作广泛模型能力覆盖。
- 在 Provider 的缓存、推理 Token、Batch/服务挡位、折扣和 usage 语义不可比时，
  发布归一化美元成本比较。
...
`DELETE` 实际执行不可恢复的安全归档。后端会锁定 Profile；若仍有排队或进行中的
运行引用它，则拒绝删除。对于已经结束但缺少身份快照的旧运行，平台先回填非敏感
身份，再清除凭据、端点和推理参数、停用并从注册表及统计中隐藏 Profile。稳定行与
历史外键继续保留，因此删除配置不会破坏或错误归属 Benchmark 证据。创建运行时也
会锁定所选 Profile，与删除操作串行化。
...
Scenario 包由独立版本化的 Suite Manifest 组织：

```text
suites/<suite-slug>/suite.yaml
  ├── families[]               相互独立的因果/任务题族
  ├── scenarios[]              不可变 slug/version 引用
  ├── split                    development | validation | held_out
  └── leaderboard policy      最少题族、隐藏题族与场景数量
```

Suite Loader 会校验全部题族引用，并确认每个 Scenario slug/version 真实存在。同一
场景的不同种子实例仍属于同一个题族；它们有助于降低污染，却不能制造任务多样性。
planned 题族不计入 active，缺失私有 held-out 集也不能被伪装成已完成。

内置「生产事故工程套件」从 0.1.0 开始，目前只引用 development 划分中的
「终焉仓库 3.0.4」。就绪策略要求五个活跃题族、三个 held-out 题族和 20 个
场景引用，因此必须报告 `leaderboard_eligible: false`。API 与 React 直接展示
这些计数。
...
一个场景可以声明多条可接受的工程路径。例如最小向前修复和受约束回滚，只要分别
满足自身因果节点以及隐藏安全/回放检查，都可以通过。判定器会返回是否有完整路径
通过、加权因果覆盖率、最佳路径覆盖率、部分分，以及缺失节点、备选组和检查的 ID。

部分覆盖率用于分析，不会自动变成通过。图节点标签、可接受路径和隐藏检查始终留在
可信场景/裁判状态中；公开 Scorecard 只暴露允许公开的 ID 与聚合值。所有节点和边
都必须引用已声明 ID，路径 ID 必须唯一，空备选组属于无效配置。

「终焉仓库 3.0.4」沿用 3.0.3 的真相与生成事故结构，并修正执行包络。通用
多路径判定器是供新场景版本和新题族使用的平台契约，不能在不改
版本号的情况下偷偷改变标准答案。
...
反复提交 Final 或触发硬预算后，循环结束，并对保留下来的部分结果评分；触及
硬预算的结果必须明确标为右删失，不能算作完成解答。
...
每条候选来源事件都带稳定 `agent_id` 与角色。派生的 Agent Graph 会聚合模型轮次、
工具调用、输入/输出 Token、首尾事件序号、状态与父子关系，但不收集私有推理。

当前内置执行器仍是单 Agent，因此只生成一个 `candidate/root` 节点。Schema 同时
接受 `agent.spawned`、`agent.delegated`、`agent.completed`、`agent.failed` 和
`agent.cancelled` 事件。外部编排器可以使用任意角色名；平台不会把
Leader/Researcher/Reviewer 写死成唯一拓扑。spawn 与 delegation 是不同的边，
因此所有权与任务分派都可以审计。

协议支持不能被宣传成内置多 Agent 调度器已经完成。候选执行、语义裁判和隐藏评分
属于不同平面；可选 Judge 不是候选 Agent 的子节点。

### 5.2 资源账本与预算

资源账本分别记录逻辑模型轮次、包含重试的真实 Provider HTTP 请求、Provider
报告的输入/输出 Token、候选工具调用，以及扣除协作式暂停后的有效执行时间。

时间、工具调用和 Provider 请求始终具有有序软/硬限制。Token 软/硬限制为可选，
因为部分本地或兼容 Provider 无法可靠返回 usage。越过软限制只发出一次可审计
告警，运行继续；越过硬限制会阻止后续候选工作，并对已有结果评分。

EvilBench 不推导归一化美元成本。缓存读写、隐藏推理 Token、Batch/服务挡位、
协商折扣与兼容 API 语义都可能显著改变账单，却不会完整出现在统一 usage 字段中。
原始用量仍可导出给掌握权威账单的运维人员，但不会混入 1,200 分主分。
...
新场景可以启用常见事故工具的确定性等价物：

| EvilBench 工具 | 熟悉的类比 | 边界 |
|---|---|---|
| `process_list` | `ps` | 仅模拟服务命名空间 |
| `service_status` | `systemctl status` | 仅回放 Manifest |
| `journal_query` | `journalctl` | 带来源的版本化日志 |
| `socket_snapshot` | `lsof` / 报文元数据 | 无真实宿主机 socket 或网络包 |
| `trace_process` | `strace` | 模拟的有界操作 |
| `profile_cpu` | `perf` | 确定性回放样本 |

所有输出都由 Incident Director 的种子、逻辑 tick、服务状态、采集器与时钟域派生。
它们可以包含高容量错误噪声、恢复监控器，或「父服务健康但子进程失败」等冲突，但
每项冲突都能复现。每次观察会像其他证据一样记录 probe 与 provenance。

这些是项目工具，不是系统命令直通。候选永远不能附加真实进程、查看宿主机或抓取
真实流量。「终焉仓库 3.0.4」不启用这组发布后新增工具；必须由新场景版本在
metadata 中显式选择。
...
Scenario 3.0.4 **终焉仓库（The Terminal Repository）** 的主体不再是一道
...
180 Tick 是逻辑状态机的回放长度，不是 180 分钟墙上时间要求。Completion Gate
要求至少推进到 Tick 140，而独立的真实执行预算为 90/180 分钟。
...
完整标准场景目标约为 5,000 个文件、2,000 次提交和 100 MiB 本机生成材料。
Scenario 3.0.4 默认软/硬预算为 90/180 分钟、600/2,200 次工具调用和
300/720 次真实 Provider 请求。Token 限制可选，但必须成对配置软/硬值。任一
软阈值都会发出一次确定性告警并记录超限遥测，任一硬阈值都会结束候选执行；
墙上时间不参与评分。每个候选默认只有
0.5 CPU、256 MiB RAM、
256 个 PID 和
...
执行版本化参考流程。90 分钟软阈值是尾部预警；180 分钟硬阈值是在收集真正成功
样本期间采用的暂定安全包络。校准报告记录墙上/有效时间、工具调用、Token、证据
覆盖、捷径尝试，以及精确的平台、场景、Provider 和模型版本。

运行时间必须按生存数据处理。只要触及时间、工具、Provider 请求或 Token 的任意
硬限制，该运行就是右删失样本：已有工作仍会评分，但 Scorecard Outcome 必须为
`budget_exhausted`，WebUI 不得称其为完成，平均得分和运行时间校准也必须排除。
只有未触及任何硬限制、满足 Completion Contract、通过隐藏验证，并达到场景配置
成功阈值的运行，才有资格参与校准；3.0.4 的阈值为 900/1,200。没有显式 Outcome
的历史 Scorecard，则根据归档的 `hard_limits_crossed` 资源账本识别。

结束执行并进入评分流水线并不代表成功。主动低分结束、耗尽预算或仅仅完成评分的
运行，绝不能再次被拿来缩短后续硬包络。

这是经验目标，不保证每个未来模型都必须耗费某个固定时长，也绝不为凑数字而延迟运行。
...
| 来源 | 在本项目中的落地 |
|---|---|
| [METR Time Horizon 1.1](https://metr.org/time-horizons/) | 人类专家任务时长与 Agent 实际墙钟时间分开记录。触及硬限制的观测按右删失处理，不能冒充完成时间；墙钟时间永不直接加分。 |
...
- 客观推理门未完成：最高 900；
- 超过硬预算：立即停止执行，对已有工作评分，并把 Outcome 标成右删失而不是完成。
...
通过/失败依据；只有 Suite 满足声明的就绪策略后，才可用于聚合排行榜。单个公开
开发场景的得分必须标成该场景结果，不能包装成通用模型排名。
...
模型配置已归档或不可用、响应无效或可靠性低都会被明确记录，但不能让原本有效的
运行失败。
...
- 登录、注册、首次管理员初始化与账户会话；
- 管理员用户、角色、注册策略与服务器监控；
- Suite 就绪状态及活跃/held-out 题族、场景引用计数；
- 场景目录与版本详情；
- 可编辑的模型/Provider 配置、服务端加密凭据、按协议映射的结构化推理参数与
  有界高级 JSON；
- 带时间、工具、真实 Provider 请求及可选 Token 软/硬限制的运行构建器；
- 实时运行矩阵与容器/资源状态；
...
- 带明确单/多 Agent 模式、角色、委派和逐 Agent 资源聚合的 Agent 执行图；
...
软删除结果是与取消不同的终态生命周期操作：排队或进行中的运行不能软删除，且
必须单独二次确认。控制平面写入 `benchmark_runs.archived_at` 和
`run.archived`，保留所有权及其全部事件、图谱、评分、产物与回放记录，同时从
用户可见入口和聚合统计中排除该行。本版不提供恢复界面；由于证据没有被物理删除，
管理员仍可从数据库恢复。
...
- 确定性真相模型；
- 带有文档化可接受解决路径和客观检查的私有 Truth Graph；
- 独立的 Suite 题族与 Split 归属；仅改变 Seed 不能创造新题族；
- 预期的非暴力搜索解题路径；
...
- Suite 发布修改题族成员、Split 归属、权重或就绪策略，但不重写引用的场景包；
