...
- 把标准题校准到能在 80 分钟硬时限内持续区分强软件工程 Agent，但绝不靠人为
  等待制造时长。
...
「终焉仓库 3.0.2」。就绪策略要求五个活跃题族、三个 held-out 题族和 20 个
...
「终焉仓库 3.0.2」保持不可变，并继续使用已发布验收语义。通用多路径判定器是供
新场景版本和新题族使用的平台契约，不能在不改版本号的情况下偷偷改变标准答案。
...
真实流量。「终焉仓库 3.0.2」不启用这组发布后新增工具；必须由新场景版本在
metadata 中显式选择。
...
要求至少推进到 Tick 140，而独立的真实执行预算仍是 40/80 分钟。
...
软/硬预算为 40/80 分钟、250/650 次工具调用和 180/360 次真实 Provider 请求。
...
执行版本化参考流程。标准目标是让强 Agent 的参考解题过程占用 80 分钟硬时限中的
...
| 来源 | 在本项目中的落地 |
|---|---|
| [METR Time Horizon 1.1](https://metr.org/time-horizons/) | 人类专家任务时长与 Agent 实际墙钟时间分开记录。强 Agent 的 80 分钟时限在多轮人类/Agent 校准完成前仍是暂定值，墙钟时间永不直接加分。 |
