...
- 把标准题校准到能在 60 分钟硬时限内持续区分强软件工程 Agent，但绝不靠人为
  等待制造时长。
...
「终焉仓库 3.0.3」。就绪策略要求五个活跃题族、三个 held-out 题族和 20 个
...
「终焉仓库 3.0.3」沿用 3.0.2 的真相与生成事故结构，并用新版本记录重新校准的
执行包络。通用多路径判定器是供新场景版本和新题族使用的平台契约，不能在不改
版本号的情况下偷偷改变标准答案。
...
真实流量。「终焉仓库 3.0.3」不启用这组发布后新增工具；必须由新场景版本在
metadata 中显式选择。
...
Scenario 3.0.3 **终焉仓库（The Terminal Repository）** 的主体不再是一道
...
要求至少推进到 Tick 140，而独立的真实执行预算为 30/60 分钟。
...
软/硬预算为 30/60 分钟、250/650 次工具调用和 180/360 次真实 Provider 请求。
Token 限制可选，但必须成对配置软/硬值。任一软阈值都会发出一次确定性告警并记录
超限遥测，任一硬阈值都会结束候选执行；墙上时间不参与评分。每个候选默认只有
0.5 CPU、256 MiB RAM、
256 个 PID 和
...
执行版本化参考流程。标准目标是让强 Agent 的参考解题过程占用 60 分钟硬时限中的
大部分时间，但仍能在时限内完成。校准报告记录墙上/有效时间、工具调用、Token、
证据覆盖、捷径尝试，以及精确的平台、场景、Provider 和模型版本。

这是经验目标，不保证每个未来模型都必须耗时 60 分钟，也绝不为凑数字而延迟运行。
...
| 来源 | 在本项目中的落地 |
|---|---|
| [METR Time Horizon 1.1](https://metr.org/time-horizons/) | 人类专家任务时长与 Agent 实际墙钟时间分开记录。强 Agent 的 60 分钟时限在多轮人类/Agent 校准完成前仍是暂定值，墙钟时间永不直接加分。 |
