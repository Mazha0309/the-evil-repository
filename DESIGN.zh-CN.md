...
编辑时省略 API Key 会保留原密钥，提供新值会替换，显式清除才会删除。删除模型
配置会销毁凭据并归档 Profile，同时在历史运行里保留冻结的非敏感模型身份。
...
普通用户只能访问映射给自己的模型配置与运行。管理员可以检查全局历史、管理用户
和注册、调整 Runner 并发，并查看经过筛选的 CPU、内存、磁盘、PostgreSQL、
队列、心跳与 Rootless Docker 容量指标。
