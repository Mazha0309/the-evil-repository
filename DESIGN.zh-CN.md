...
Runner 还拥有一个确定性、只触发一次的最终收尾窗口。任一受监控资源达到其软阈值
与硬阈值 80% 两者中较晚的位置后，Runner 会注入一条可信消息，列出剩余预算和
当前完成门缺口，要求候选模型收敛并进行自我验证，但不会改变完成门、分数或硬
边界。该注入记录为 `run.finalization_nudge`，可在实时遥测与运行归档中查看。
